...
def mudpit(level_st, rho_st, rho_in, inflow, outflow, mud, water, dTime, st, rmt):
    ...
    rhoM = m.Const(value=980.0)              # makeup mud density (kg/m3)
    rhoW = m.Const(value=1000.0)              # water density (kg/m3)
    #pitV = m.Const(value=10.0)                # Mud pit volume (m3)
    pitA = m.Const(value=10)                 # Mud pit area (m2)
...
def mpc(cv_m, cv_l, cv_h, 
    ...
    Qbp_new = Qbp_m + bp_gain * (hpit_m - (hpit_l+hpit_h)/2)
    Qbp_new = min(max(Qbp_new, 0.01), 10.0)
        
#    if cv_new < 15:
#        Qbp_new = min(Qbp_m + 1, 0)
#    elif cv_new > 50:
...
#    bp_gain = 1
#    if cv_new < 20:
#        Qbp_new = Qbp_m + bp_gain * (20 - cv_new)
#    elif cv_new > 30:
#        Qbp_new = Qbp_m + bp_gain * (30 - cv_new)
#    else:
#        Qbp_new = Qbp_m
#
...
plt.subplot(3,2,1)
plt.plot(telapsed_[2:] / 3600, md_[2:], 'r-', label='Measured depth')
plt.plot(telapsed_[2:] / 3600, tvd_[2:], 'b-', label='True vertical depth')
plt.ylabel('Depth (m)')
...
plt.subplot(3,2,1)
plt.plot(md_[2:], md_[2:], 'r-', label='Measured depth')
plt.plot(md_[2:], tvd_[2:], 'b-', label='True vertical depth')
plt.ylabel('Depth (m)')
plt.gca().invert_yaxis()
plt.legend(loc='upper right')

plt.subplot(3,2,2)
plt.plot(md_[2:], Pmp_[2:], 'k-', label='Mud pump discharge pressure')
plt.plot(md_[2:],Pdh_[2:], 'r-', label='Drill bit pressure')
plt.plot(md_[2:], Pf_[2:], 'g:', label='Formation pressure')
plt.plot(md_[2:], Pchoke_[2:], 'b-', label='Choke valve inlet pressure')
plt.ylabel('Pressure (bar)')
plt.legend(loc='upper left')

plt.subplot(3,2,3)
plt.plot(md_[2:], Qbit_[2:], 'k-', label='Drill bit flowrate')
plt.plot(md_[2:], Qres_[2:], 'g:', label='Formation flowrate')
plt.plot(md_[2:], Qchoke_[2:], 'b-', label='Choke flowrate')
plt.plot(md_[2:], Qbp_[2:], 'y-', label='Back-pressure pump flowrate')
plt.ylabel('Flow (m3/min)')
plt.legend(loc='upper left')

plt.subplot(3,2,4)
plt.plot(md_[2:], k_[2:], 'k-', label='Permeability')
plt.yscale('log')
plt.ylabel('Permeability')
plt.legend(loc='upper left')

plt.subplot(3,2,5)
plt.plot(md_[2:], hpit_[2:], 'k-', label='Pit Level')
plt.ylabel('Height (m)')
plt.legend(loc='upper left')
plt.xlabel('Measured Depth (m)')

plt.subplot(3,2,6)
plt.plot(md_[2:], cv_[2:], 'b-', label='Choke Position')
plt.ylabel('%')
plt.legend(loc='upper left')
